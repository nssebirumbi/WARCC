...
File myFile;
int rep_count = 1, delcount = 0, ind = 0, filesize = 0;
double SOC = 100.0;
bool newdata = false, client_connected = false, cellular_is_on = false, fuelGaugeIsAsleep = false;
/*increments sleep periods during low power conditions*/
int n = 0;
		...
		while ((d = myFile.read()) >= 0) {
			...
			if(d == '\n') {
				...
				if(temp.indexOf("SERVER") > -1) {
					...
					/*remove the character at the last index if its a null terminator*/
...
void loop() {
	...
	if(rep_count >= reps && SOC >= 15.0) {
		...
		//get the time we turned on the module
		lastTime = millis();
		cellular_is_on = true;
		Cellular.connect();
		...
		//rough assumption. all data was sent
		if(client_connected) {
