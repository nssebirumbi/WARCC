...
var k,
  cp = 0,
  ii; // cp is a flag to tell when we are looping through commands paert of data.
var commands = "";
...
net
  .createServer(function (socket) {
    ...
    socket.on("data", function (data) {
      ...
      if (strdata.startsWith("To ")) {
        //EXPECTED FORMAT: "To station-clientx data-to-send-to-the-client"
      ...
      if (strdata.startsWith("ToAll ")) {
        //EXPECTED FORMAT: "ToAll client1,client2,cl3 the-data-to-send-to-them" or "ToAll all the-data-to-send-to-them"
      ...
      if (strdata.startsWith("mac ")) {
        //EXPECTED FORMAT: "login username:user password:pass"
        var stationData = strdata.split(" ")[1]; //station:makerere
        var station = stationData.split(":")[1];
        var nodeName = strdata.split(" ")[2]; //node:10m
        var node = nodeName.split(":")[1];
        //Read json file and fetch station and nodemac address

        let stringdata = fs.readFileSync("./node.json");
        let stations = JSON.parse(stringdata);

        if (stations[station] &&stations[station].station == station ) {
          if (node==="10m") {
            socket.write("E64="+stations[station].ten);
          }else if (node==="2m") {
            socket.write("E64="+stations[station].two);
          }else if (node==="gnd") {
            socket.write("E64="+stations[station].gnd);
          }
        } else {
          socket.write("mac=failed");
        }
      }

      if (strdata.startsWith("register ")) {
        //EXPECTED FORMAT: "register username:user password:pass"
    ...
    function broadcast(reqData, targetClients, sender) {
      if (targetClients === "all") {
        clients.forEach(function (client) {
          if (client.identity !== sender.identity) {
